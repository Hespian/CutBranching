...
const int WORK_OP_RELABEL_2 = 9;
const double GLOBAL_UPDATE_FRQ_2 = 0.51;
const int WORK_NODE_TO_EDGES_2 = 4;
...
public:
                                    ...
                                    std::vector<NodeID> &source_set)
    {
        ...
        int work_todo = WORK_NODE_TO_EDGES_2 * G.number_of_nodes() + G.number_of_edges();
        // main loop
        while (!m_Q.empty())
        {
            NodeID v = m_Q.front();
            ...
            if (m_work > GLOBAL_UPDATE_FRQ_2 * work_todo)
            {
                global_relabeling(source, sink);
                m_work = 0;
                m_global_updates++;
