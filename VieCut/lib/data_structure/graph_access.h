...
class graph_access {
 ...
 public:
    ...
    int build_from_adj(std::vector<std::vector<int>>& adj);
...
inline int graph_access::build_from_adj(std::vector<std::vector<int>>& adj)
{
    int n = adj.size();
    int m = 0;
    ...
    for (int i = 0; i < n; i++) {
        ...
        for (int j = 0; j < adj[i].size(); j++) {
            EdgeID e_bar = new_edge(node, adj[i][j]);
            setEdgeWeight(e_bar, 1);
        }
    }
