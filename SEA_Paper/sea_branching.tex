...
\relatedversiondetails{Full Version}{https://arxiv.org/abs/2102.01540}
...
rules when branching. Akiba and Iwata apply the reduction rules in a predefined
order. For each rule, their algorithm iterates over all
vertices in the graph and checks whether the rule
...
that decompose the graph into connected components. We use this algorithm
...
identical machines and run at most 8 instances at once on the
...
From these instances, we excluded all instances that (1) required no branches, (2) on which all techniques had a running time of less than $0.1$ seconds, or (3) on which no technique was able to find a solution within 10 hours.
...
instances, we achieve a total speedup\footnote{calculated by dividing the running times
...
However, the running time of this approach still suffers from frequent checks whether a particular vertex is a potential branching vertex.
...
A value is highlighted in bold if it is the best one within a row.
