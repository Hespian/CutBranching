...
degree that also minimizes the number of edges in its neighborhood\todo{Does this mean between its neighbors or in the 2-neighborhood?} is optimal
...
is large enough\todo{sounds strange to me. sufficiently large?} (depending on the maximum degree of the graph) by
...
\todo{Mention the results of Gao et al. (https://www.ijcai.org/Proceedings/2018/0201.pdf). However its for k-plexes.}
...
computational overhead.\todo{Are the candidates dynamically updated similar to the previous strategies?} To be self contained we will also repeat the reduction
...
of maximum degree.\todo{What if no candidates were found?} The rational here is that a vertex of large degree changes
the structure of the graph more than a vertex of small degree even if that
vertex is guaranteed to enable the application of a reduction rule.\todo{Mention somewhere that these rules really only make sense in the excluding branch?}
...
reduction. Finding almost twins can be done while searching for twins:\todo{explicitly mention that this only has constant overhead?} The
...
\{u, w'\}$ induce a clique.\todo{Mention overhead? Same goes for the following reduction rules}
...
Again, we define a condition for being almost dominated:\todo{Maybe use different wording? Sounds repetitive}
