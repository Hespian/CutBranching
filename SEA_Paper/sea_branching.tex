...
as $N(U) = \cup_{u \in U} N(v)$\todo{we should check whether we have to exclude
  $U$ here} and $N[U] = N(U) \cup U$, respectively.
...
$G[V_S] = (V_S, E_S)$\todo{is this notation ever used?} is given by restricting the edges of $G$ to vertices of $V_S$, \ie $E_S = \{\{u,v\} \in E \mid u,v \in V_S\}$.
Likewise, for a subset of edges $E_S \subseteq E$, the \emph{edge-induced
  subgraph}\todo{do we use edge induced subgraphs?} $G[E_S] = (V_S, E_S)$ is given by restricting the vertices of $G$ to the endpoints of edges in $E_S$, \ie $V_S = \{u \in V \mid \{u,v\} \in E_S\}$.
...
\todo{In Reduktionen benutzen wir noch $G - \{\text{Knoten}\}$}
...
$v$.\todo{by this definition the root is always an articulation point}
...
vertex.\todo{why not the one with the highest degree?}
...
frequently.\todo{Parameter maximale Rekursionstiefe und Anzahl ausgelassene Rekursionen?}
...
vertices\todo{more precisely? Random vertices of maximum degree?} for $s$ and $t$ seems to produce the best results.
Finally, to derive a vertex separator from a cut one can compute an MVC on the bipartite graph induced by the cut set, \eg using the Hopcroft-Karp algorithm.\todo{citation?}
...
ordering (while also skipping non-separator vertices\todo{I'm confused. Where do
non-separator vertices come from?}), we continuously branch on vertices that disconnect the graph into balanced partitions. 
...
separators considered during branching using a threshold.\todo{aren't all
...
\begin{theorem} (Alternative Reduction~\cite{XiaoUnconfined})
    ...
    (E\setminus\binom{A\cup B\cup (N(A)\cap N(B))}{2} \cup \{ \{x,y\}\;|\; x\in
    N(A)\setminus N[B], y\in N[B]\setminus N(A) \}$ and let $I'$ be an MIS in $G'$. Then,
	$
    I=\begin{cases}
