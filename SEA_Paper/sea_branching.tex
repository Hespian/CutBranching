...
For a subset of vertices $V_S \subseteq V$, the \emph{(vertex-)induced subgraph} $G[V_S] = (V_S, E_S)$ is given by restricting the edges of $G$ to vertices of $V_S$, \ie $E_S = \{\{u,v\} \in E \mid u,v \in V_S\}$.
Likewise, for a subset of edges $E_S \subseteq E$, the \emph{edge-induced subgraph} $G[E_S] = (V_S, E_S)$ is given by restricting the vertices of $G$ to the endpoints of edges in $E_S$, \ie $V_S = \{u \in V \mid \{u,v\} \in E_S\}$.
...
number of vertices from this set that have to be included in a maximum
independent set. This information can be used when branching on a vertex
...
Thus, if we select branching vertices based on the reverse nested dissection ordering (while also skipping non-separator vertices), we continuously branch on vertices that disconnect the graph into balanced partitions. 
...
Furthermore, similar to the edge cut based strategy, we limit the size of separators considered during branching using a threshold.
Again, this is done to ensure that we do not require to many branching steps to decompose the graph.
...
publications. Out of the candidates found we then select the vertex of
...
\begin{theorem} (Twin Reduction~\cite{XiaoUnconfined}) In a graph $G=(V,E)$ let vertices $u$ and $v$ be twins. If there is an edge among $N(u)$, then there is always a maximum independent set that includes $\{u,v\}$ and therefore excludes $N(u)$. Otherwise, let $G'=(V',E')$ be the graph with $V'=(V\setminus N[\{u,v\}])\cup\{w\}$ where $w\notin V$ and $E'=(E\cap\binom{V'}{2})\cup \{\{w,x\}\;|\;x\in N^2(u)\})\}$ and let $I'$ be a \textit{maximum vertex cover} in $G'$. Then, 
  \[I=\begin{cases}
      I'\cup \{u,v\} & \text{, if }w\notin I'\\
...
\begin{theorem} (Dominance Reduction~\cite{Fomin}) In a graph $G=(V,E)$, if a vertex $u$ is dominated by a neighbor $v$, then, there always exists a maximum independent set that does not include $v$, i.e.\[\alpha(G)=\alpha(G-v)\]
\end{theorem}
...
The core idea is to detect vertices not required for a maximum independent set
that can therefore can be removed from the graph by algorithmically contradicting the assumption that every maximum independent set contains the vertex.

\begin{definition} (Removable Vertex~\cite{XiaoNagamochi}\todo{Was meint ihr? Macht es
    hier Sinn Xaio und Nagamochi zu zitieren oder ist das zu banal?})
...
\begin{theorem}(Unconfined Reduction~\cite{XiaoNagamochi}) In a graph $G=(V,E)$, if Algorithm \ref{alg:unconf} returns true for an unconfined vertex $v$, then there is always a maximum independent set that does not contain~$v$.
\end{theorem}
