...
Finally, to derive a vertex separator from a cut one can compute a MVC on the bipartite graph induced by the cut set, \eg using the Hopcroft-Karp algorithm.
...
\begin{theorem} (Alternative Reduction~\cite{XiaoUnconfined})
    ...
    N(A)\setminus N[B], y\in N[B]\setminus N(A) \}$ and let $I'$ be an MIS set in $G'$. Then,
	\[I=\begin{cases}
