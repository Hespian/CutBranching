...
\begin{abstract}
  Finding a maximum independent set is a fundamental NP-hard problem that is used in many real-world applications~\cite{BIO2,BIO1,RP,NW,CG}.
...
branch-and-reduce algorithm by up to \textcolor{red}{$X$}\todo{insert final
  number} percent.\todo{nicht klar was hier gemeint ist.}
In particular our reduction rule based strategies are able to outperform the default branching strategy on \textcolor{red}{$X$}\todo{insert final number} percent of the instances tested.
...
seconds.\todo{davon haben wir halt nur eine einzige}
