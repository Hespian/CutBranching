...
For this purpose we make use of \emph{nested dissection ordering}.

A nested dissection ordering of the vertices of a graph $G$ is obtained by recursively computinig balanced bipartitions $(A,B)$ and a vertex separator $S$.
The actual ordering is then given by concatenating the orderings of $A$ and $B$ followed by the vertices of $S$.
Thus, if we select branching vertices based on the reverse nested dissection ordering (while also skipping non-separator vertices), we continiously branch on vertices that disconnect the graph into balanced partitions. 
...
First, we noticed that vertices at the end of the ordering seldomly lead to a decomposition of the graph. 
...
Finally, if this threshold is exceeded, the default branching strategy is used.
