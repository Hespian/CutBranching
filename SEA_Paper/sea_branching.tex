...
$\sum_{u\in N(v)}x_u \geq2$,\todo{Diese Regel verstehe ich nicht} where $x_u$ is a binary variable that indicates
whether a vertex is included in the current solution. The algorithm creates such
...
vertices from $S$ have to be included into the current solution\todo{kein check,
ob es eine Kante innerhalb von $S$ gibt?}. Secondly, if there is a vertex $v$ such that $|S|-|N(v)\cap S| < k$, then $v$ has to be excluded from the current solution. If $k > |S|$, the constraint can not be fulfilled and the current branch is pruned.
...
to $\numprint{0.98}$ to $\numprint{1.04}$.\todo{Remove this?} Table~\ref{tab:summary_decomp} shows that overall, our
...
techniques and maximum degree branching.\todo{remove?}
...
\begin{subfigure}[t!]{\textwidth}
	\centering
	\input{plots/decomposition_based_time.tex}
...
\begin{subfigure}[t!]{\textwidth}
	\centering
	\input{plots/reduction_based_time.tex}
